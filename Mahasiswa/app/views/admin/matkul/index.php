                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php if (!empty($matkuls)): ?>
                            <?php $no = 1; foreach ($matkuls as $mk): ?>
                                <tr>
                                    <td><?= $no++ ?></td>
                                    ...
                                    <td><?= $mk['sks'] ?> SKS</td>
                                    <td>
                                        <a href="index.php?page=matkul&action=edit&id=<?= $mk['id'] ?>" class="btn btn-secondary btn-sm">Edit</a>
                                        <a href="index.php?page=matkul&action=hapus&id=<?= $mk['id'] ?>" class="btn btn-danger btn-sm"
                                           onclick="return konfirmasiHapus('<?= htmlspecialchars($mk['nama']) ?>')">Hapus</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="5" class="text-center text-muted">Belum ada mata kuliah.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
