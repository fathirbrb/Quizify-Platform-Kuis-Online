        ...
        <section class="page-content">
            <div class="kuis-filter fade-in">
                ...
                ?>
                    <a href="index.php?page=kuis-tersedia&filter=<?= $val ?>"
            ...
            <?php else: ?>
                <div class="kuis-grid stagger">
                    <?php foreach ($kuisList as $kuis): ?>
                        ...
                        $badgeText  = 'Dibuka';
                        if ($status === 'terjadwal') { $badgeClass = 'badge-scheduled'; $badgeText = 'Terjadwal'; }
                        if ($status === 'selesai')   { $badgeClass = 'badge-done';      $badgeText = 'Selesai'; }
                        ?>

                        <div class="kuis-card">
                            <div class="kuis-card-img img-<?= htmlspecialchars($warna) ?>">
                                <span class="kuis-badge <?= $badgeClass ?>"><?= $badgeText ?></span>
                                <span class="kuis-qs"><?= (int)$kuis['jumlah_soal'] ?> Soal</span>
                            </div>
                            <div class="kuis-card-body">
                                <p class="kuis-matkul"><?= htmlspecialchars($kuis['nama_matkul']) ?></p>
                                <p class="kuis-title"><?= htmlspecialchars($kuis['nama_kuis']) ?></p>
                                <div class="kuis-meta-row">
                                    <span>⏱ <?= (int)$kuis['durasi'] ?> menit</span>
                                    <span>
                                        <?php if ($status === 'selesai'): ?>
                                            Nilai: <?= (int)$kuis['nilai'] ?>
                                        <?php elseif ($status === 'terjadwal'): ?>
                                            Besok
                                        <?php else: ?>
                                            Batas <?= htmlspecialchars($kuis['batas_waktu']) ?>
                                        <?php endif; ?>
                                    </span>
                                </div>
                            ...
                            <div class="modal">
                                ...
                                <div class="modal-header img-<?= htmlspecialchars($warna) ?>">
                                    <div class="modal-header-content">
                                        <span class="kuis-badge <?= $badgeClass ?>"><?= $badgeText ?></span>
                                        <span class="kuis-qs" style="position:static"><?= (int)$kuis['jumlah_soal'] ?> Soal</span>
                                    </div>
                                </div>
                                <div class="modal-body">
                                    <p class="kuis-matkul"><?= htmlspecialchars($kuis['nama_matkul']) ?></p>
                                    <p class="modal-title"><?= htmlspecialchars($kuis['nama_kuis']) ?></p>
                                    <div class="info-grid">
                                        <div class="info-card"><label>Durasi</label><span><?= (int)$kuis['durasi'] ?> menit</span></div>
                                        <div class="info-card"><label>Jumlah Soal</label><span><?= (int)$kuis['jumlah_soal'] ?> soal</span></div>
                                        <?php if ($status === 'selesai'): ?>
                                            <div class="info-card"><label>Nilai Kamu</label><span style="color:#16a34a;font-size:1.2rem"><?= (int)$kuis['nilai'] ?></span></div>
                                        ...
                                        <?php else: ?>
                                            <div class="info-card"><label>Batas Waktu</label><span><?= htmlspecialchars($kuis['batas_waktu']) ?> WIB</span></div>
