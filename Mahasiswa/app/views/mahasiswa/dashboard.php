        ...
        <section class="page-content">
            ...
            <div class="stats-grid stagger">
                <div class="stat-card orange">
                    ...
                    <div class="stat-value"><?= $stats['kuis_tersedia'] ?></div>
                </div>
                ...
                <div class="stat-card blue">
                    ...
                    <div class="stat-value"><?= $stats['nilai_terakhir'] ?></div>
                </div>
            </div>
                    ...
                    <?php foreach ($aktivitas as $item): ?>
                        <div class="activity-card">
                            <div class="activity-card-img img-<?= htmlspecialchars($item['warna'] ?? 'blue') ?>">
                                <span class="activity-badge"><?= htmlspecialchars($item['status'] ?? 'Tersedia') ?></span>
                                <span class="activity-qs"><?= (int)($item['jumlah_soal'] ?? 0) ?> Soal</span>
                            </div>
                            <div class="activity-card-body">
                                <p class="activity-card-title"><?= htmlspecialchars($item['nama_kuis'] ?? '') ?></p>
                                <p class="activity-card-sub">
                                    <?= htmlspecialchars($item['nama_matkul'] ?? '') ?> · <?= (int)($item['durasi'] ?? 0) ?> menit
                                </p>
                                <?php if (!empty($item['nilai'])): ?>
                                    <span class="activity-acc"><?= (int)$item['nilai'] ?>% akurasi</span>
                                <?php else: ?>
