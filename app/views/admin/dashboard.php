...
<main class="main">
    ...
    <section class="page-content">
        ...
        <div class="card">
            <table class="table">
                ...
                <tbody id="admin-activity-body" data-feed-url="index.php?page=admin-activity-feed">
    ...
    (() => {
        ...
        if (!body) return;
        let signature = JSON.stringify(<?= json_encode(array_values($aktivitas ?? [])) ?>);
        ...
        const render = (logs) => {
            if (!logs.length) {
                body.innerHTML = '<tr><td colspan="5" class="text-center text-muted">Belum ada aktivitas tercatat.</td></tr>';
                return;
            }

            body.innerHTML = logs.map((log) => `
                <tr>
                    <td>${esc(log.nama || 'Tidak diketahui')}</td>
