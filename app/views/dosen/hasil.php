...
<section class="page-content">
    <div class="section-header" style="margin-bottom:0.75rem;">
        <div>
            <h2>Laporan Nilai</h2>
            <p class="text-muted">Pantau rekap nilai mahasiswa berdasarkan kelas dan mata kuliah yang Anda ampu.</p>
        </div>
    </div>

    <!-- Breadcrumbs Navigasi -->
    <nav class="breadcrumb" style="margin-bottom:1.5rem;">
        <a href="index.php?page=dosen-hasil" style="text-decoration:none; display:inline-flex; align-items:center; gap:4px;">
        ...
        <?php if ($selectedKelasId !== null): ?>
            ...
            <?php if ($selectedMatkulId !== null): ?>
                <a href="index.php?page=dosen-hasil&kelas_id=<?= urlencode((string)$selectedKelasId) ?>" style="text-decoration:none;">
    ...
    <?php if ($selectedKelasId === null): ?>
        ...
        <div class="card" style="padding:1.5rem; margin-bottom:1.5rem; border-left: 4px solid var(--accent);">
            ...
            <p class="text-muted" style="margin: 0.25rem 0 0; font-size:0.85rem;">Pilih kelas di bawah ini untuk melihat laporan nilai mahasiswa.</p>
        </div>
        ...
        <?php if (!empty($kelasDiampu)): ?>
            <div class="kelas-grid">
                ...
                <?php foreach ($kelasDiampu as $index => $kelas): ?>
                    ...
                    <a href="index.php?page=dosen-hasil&kelas_id=<?= urlencode((string)$kelas['kelas_id']) ?>" class="kelas-card card" style="transition: transform 0.2s, box-shadow 0.2s;">
    ...
    <?php elseif ($selectedMatkulId === null): ?>
        ...
        <div class="card" style="padding:1.5rem; margin-bottom:1.5rem; border-left: 4px solid var(--accent); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:1rem;">
            <div>
                ...
                <p class="text-muted" style="margin: 0.25rem 0 0; font-size:0.85rem;">Pilih mata kuliah di kelas <strong><?= htmlspecialchars($selectedKelas['kelas'] ?? '') ?></strong> untuk menampilkan laporan nilai.</p>
            </div>
            <a href="index.php?page=dosen-hasil" class="btn btn-secondary btn-sm" style="display:inline-flex; align-items:center; gap:4px;">
        ...
        <?php if (!empty($filteredMatkul)): ?>
            <div class="kelas-grid">
                ...
                <?php foreach ($filteredMatkul as $index => $mk): ?>
                    <?php $color = $colors[$index % count($colors)]; ?>
                    <a href="index.php?page=dosen-hasil&kelas_id=<?= urlencode((string)$selectedKelasId) ?>&matkul_id=<?= urlencode((string)$mk['matkul_id']) ?>" class="kelas-card card" style="transition: transform 0.2s, box-shadow 0.2s;">
                        <div class="kelas-card-header kelas-<?= $color ?>">
                            <div class="kelas-card-icon">
    ...
    <?php else: ?>
        <!-- LANGKAH 3: TABEL LAPORAN NILAI -->
        <div class="card" style="padding:1.5rem; margin-bottom:1.5rem; border-left: 4px solid var(--accent); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:1rem;">
            <div>
                <h3 style="margin-top:0; font-size:1.05rem; font-weight:700;">Langkah 3: Nilai Rekap Mahasiswa</h3>
                <p class="text-muted" style="margin: 0.25rem 0 0; font-size:0.85rem;">
                    Menampilkan laporan nilai pada mata kuliah <strong><?= htmlspecialchars($selectedMatkul['nama_matkul'] ?? '') ?></strong> &bull; Kelas <strong><?= htmlspecialchars($selectedKelas['kelas'] ?? '') ?></strong>.
                </p>
            </div>
            <a href="index.php?page=dosen-hasil&kelas_id=<?= urlencode((string)$selectedKelasId) ?>" class="btn btn-secondary btn-sm" style="display:inline-flex; align-items:center; gap:4px;">
                <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><polyline points="15 18 9 12 15 6"></polyline></svg>
        ...
        <div class="stats-grid" style="margin-bottom: 1.5rem;">
            <div class="stat-card green">
                ...
                <div class="stat-value"><?= (int)$stats['tertinggi'] ?></div>
            </div>

            <div class="stat-card orange">
                <div class="stat-label">Nilai Terendah</div>
                <div class="stat-value"><?= (int)$stats['terendah'] ?></div>
            </div>

            <div class="stat-card blue">
                <div class="stat-label">Rata-rata Nilai</div>
                <div class="stat-value"><?= (float)$stats['rata_rata'] ?></div>
            </div>

            <div class="stat-card purple">
                <div class="stat-label">Kelulusan</div>
                <div class="stat-value"><?= (int)$stats['kelulusan'] ?>%</div>
            </div>
        </div>
        ...
        <div class="card">
            <table class="table">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" class="empty-row">Belum ada nilai untuk mata kuliah dan kelas ini.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
